    ...
    methods: {
                ...
                if (this.checkPosition(x, y, size, canvasWidth, canvasHeight)) {
                  ...
                  if (x + size > canvasWidth) {
                    ...
                    if (y + size > canvasHeight) {
                      noSpaceAvailable = true; // Set flag if no space is available
                      break; // Exit while loop if no space is available
                    }
                  }
                }
              }
      ...
      addMouseEvents() {
        ...
        canvas.addEventListener('mousemove', (event) => {
          ...
          const hoveredSquare = this.occupiedPositions.find(pos =>
            x >= pos.x &&
            x <= pos.x + pos.size &&
            y >= pos.y &&
            y <= pos.y + pos.size
        ...
        canvas.addEventListener('click', (event) => {
          ...
          const clickedSquare = this.occupiedPositions.find(pos =>
            x >= pos.x &&
            x <= pos.x + pos.size &&
            y >= pos.y &&
            y <= pos.y + pos.size
